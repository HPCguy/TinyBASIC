...
#define NUM_CONST 16 /* don't change this value */
#define NUM_LAB   100
#define LAB_LEN   10
#define LOOP_NEST 25
#define SUB_NEST  25
...
int get_token()
{
  ...
  if (*prog & 0x80) { /* tokenized action */
    ...
    if (islower(bytecode)) {
      ...
      token_type = NUMBER;
      token[0] = '0'; /* dummy numeric value */
      if (bytecode != FINISHED+10) { /* sigle digit */
        token_value = bytecode - FINISHED;
        skip_token();
      }
      else { /* multiple digit */
        bytecode = prog[1];
        token_value = lit_table[bytecode & 0x0f];
        prog += (bytecode >> 4) & 0x0f;
      }
    }
    else {
       /* delimiter */
...
EOL_case:
    ...
    if (*prog == ' ') ++prog;  /* skip over white space */
    ...
    lastTokPos = prog ; /* more aggressive position, if possible */
...
/* tokenize code to speed lexer */
void tokenize()
{
  char *temp = prog;   /* save pointer to top of program */
  char *prevTokPos = 0;
  int saveValue = 0;

  /* tokenize some symbols for quick parsing */
  do {
    get_token();

    if (prevTokPos != 0) {
      int bytesToSkip = lastTokPos - prevTokPos;
      if (saveValue == 0) {  /* for COMMAND tokens */
        /* store fast forward offset to current token */
        prevTokPos[1] = bytesToSkip;
      }
      else { /* for NUMBER tokens */
        if (bytesToSkip < 16) {
          int i;
          for (i=0; i<num_lit; ++i) {
            if (lit_table[i] == token_value) break;
          }
          if (i != NUM_CONST) {
            if (i == num_lit) {
              lit_table[i] = token_value;
              ++num_lit;
            }
            prevTokPos[0] = FINISHED+10 | 0x80;
            prevTokPos[1] = i | bytesToSkip << 4;
          }
        }
        saveValue = 0;
      }
    ...
    if (token_type == COMMAND) {
      ...
      *lastTokPos |= 0x80;
      ...
      if (token_value < 10) {
        *lastTokPos = FINISHED + token_value | 0x80;
      }
      else {
        saveValue = token_value;
        prevTokPos = lastTokPos;
      }
    }
  } while(tok!=FINISHED);
